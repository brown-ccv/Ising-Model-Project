...
#takes in a Float sd that represents the standard deviation
function gaussian_rf(N, sd)
    nd = Normal(0, sd)
    return rand(nd, N)
  end
...
#Ising Model
function get_energy(s, h, J, N::Int)
    E1 = 0.0
    E2 = 0.0
    for i = 1:N-1
        for j = i+1:N
...
function do_MC_Step(config, h, kT, J, N::Int)
    E = get_energy(config, h, J, N)
    for i = 1:N
        site = rand(1:N)
...
function metropolis(config0, h, kT, J, mcsteps::Int, N)
    # config = copy(config0)
...
function execute(initkT::Float64, iter::Float64, finalkT::Float64, config, h, J::Float64, N::Int, mcsteps::Int)
    function mapred(col, kT, J, mcsteps, N)
        mag, mag_sq = metropolis(col, h, kT, J, mcsteps, N)
...
function main()
    ...
    h = gaussian_rf(Nspins, sd)

    X_list, Msq_list = execute(initkT, iter, finalkT, config0, h, J, Nspins, mcsteps)

    out = (kT=collect(initkT:iter:finalkT), X=X_list, Msq=Msq_list, Nspins=Nspins, mcsteps=mcsteps, seed=seed)

    # write X_list1 to csv file
    path = joinpath(@__DIR__, "results--Nspins-$Nspins--mcsteps-$mcsteps--sd-$sd", "result.jld")
    mkpath(dirname(path))
    serialize(path, out)

    if plotting
        p = plot(initkT:iter:finalkT, X_list, xlabel="Temperature (kT)", ylabel="Magnetic Susceptibility", linewidth=2.5, label="h = 0")
