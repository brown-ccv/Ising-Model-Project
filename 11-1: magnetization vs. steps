...
#for j = 1:2
  ...
  #a new array of magnization into this new variable
    #push!(avg_mags, last(data[2]))

    #display(plot!(1:data[1], data[2], legend = false))
...
#println("magnetization of final state: ", last(mags_list))
  #avg_mag = sum(avg_mags)/runs


plot(1:steps+1, data[2], xlabel = "Steps", ylabel = "Magnetization")
#for i=1:runs
...
#avg_mags = Vector{Float64}()
#for i=1:runs
  #display(plot!(1:states_list[i], mags_list[i], linecolor = :blue, legend = false))
  #push!(avg_mags, last(mags_list[i]))
  #println("number of steps: ", states_list[i])
#end
#avgm1 = sum(avg_mags)/runs
#empty!(avg_mags)

#for i=runs+1:runs*2
  #display(plot!(1:states_list[i], mags_list[i], linecolor = :red, legend = false))
  #push!(avg_mags, last(mags_list[i]))
  #println("number of steps: ", states_list[i])
#end
#avgm2 = sum(avg_mags)/runs

#avg_X = (avgm2 - avgm1)/m_factor
#this should give avg susceptibility
#since dH = (2*m_factor - 1*m_factor)

#print("Average Susceptibility: ", avg_X)
