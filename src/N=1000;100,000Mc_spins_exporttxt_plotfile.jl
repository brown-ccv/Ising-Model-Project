...
# Generate x-y data for gaussian
x_data = initkT:iter:finalkT
y_data = X_list1
y1_data = X_list2

# Write x-y data to a text file
output_file = "N=1000,Pure_state,100,000_MC_steps_output.txt"
output_file1= "N=1000,Gaussian,100,000_MC_steps_output.txt"
write_xy_data_to_file(output_file, x_data, y_data)
write_xy_data_to_file(output_file1, x_data, y1_data)
...
function read_xy_data(filename, delimiter='\t')
    data = readdlm(filename, delimiter)
    x_data = data[:, 1]
    y_data = data[:, 2]
    return x_data, y_data
end
filename = "N=1000,Pure_state,100,000_MC_steps_output.txt" # Replace with the name of your text file
filenames = "N=1000,Pure_state,100,000_MC_steps_output.txt" # Replace with the name of your text file
x_data, y_data = read_xy_data(filename)
x1_data, y1_data = read_xy_data(filenames)
plot!(x_data, y_data, xlabel = "Temperature", ylabel = "Susceptibility")
plot!(x_data, y1_data, xlabel = "Temperature", ylabel = "Susceptibility")
